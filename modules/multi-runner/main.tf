locals {
  ...
  runner_default_labels_enabled = { for k, v in var.multi_runner_config : k => v.matcherConfig.enableDefaultLabels }

  runner_config = { for k, v in var.multi_runner_config : k => merge({ id = aws_sqs_queue.queued_builds[k].id, arn = aws_sqs_queue.queued_builds[k].arn }, merge(v, { runner_config = merge(v.runner_config, { runner_extra_labels = local.runner_extra_labels[k] }, { runner_default_labels_enabled = local.runner_default_labels_enabled[k] }) })) }
...
output "default_labels_enabled" {
  value = local.default_labels_enabled
}
