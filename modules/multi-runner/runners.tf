module "runners" {
  ...
  runner_labels                        = each.value.runner_config.runner_default_labels_enabled ? sort(distinct(concat(["self-hosted", each.value.runner_config.runner_os, each.value.runner_config.runner_architecture], each.value.runner_config.runner_extra_labels))) : sort(distinct(each.value.runner_config.runner_extra_labels))
